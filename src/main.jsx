...
function LocationPage() {
  ...
  return <><PageHero eyebrow="Location" title="Visit Compustar at Game City Mall." text="Find the store upstairs at Game City Mall in Gaborone for product enquiries, repairs, accessories, and practical technology support." /><section className="location-page section"><div className="location-card" data-reveal><p className="kicker">Store Location</p><h2>Shop 6U, upstairs.</h2><p><MapPin /> {location}</p><p><Phone /> {displayPhone}</p><p><Mail /> {email}</p><a className="button dark" href={`https://www.google.com/maps/search/?api=1&query=${mapQuery}`} target="_blank" rel="noreferrer">Open in Google Maps <ArrowRight size={16} /></a></div><div className="map-shell" data-reveal><iframe title="Compustar location map" loading="lazy" referrerPolicy="no-referrer-when-downgrade" src={`https://www.google.com/maps?q=${mapQuery}&z=17&output=embed`}></iframe></div></section></>;
}
